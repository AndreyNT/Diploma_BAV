...
class HtmlOutput:
    ...
    def generateTimeTable(solution, slot_table):
        ...
        for cc, reservation in items():
            ...
            indexId = (dayId - 1) * sectionNum + (1 if section == 0 else section)

            key = (periodId, year, specialization)
            if key not in slot_table:
                year_duration = [0] * (ROOM_COLUMN_NUMBER * sectionNum - sectionNum + 1)
            ...
            for m in range(1, dur):
                ...
                if next_key not in slot_table:
                    slot_table[next_key] = [0] * (ROOM_COLUMN_NUMBER * sectionNum - sectionNum + 1)
                if slot_table[next_key][indexId] < 1:
                    slot_table[next_key][indexId] = -1

            if key not in time_table:
                year_schedule = [None] * (ROOM_COLUMN_NUMBER * sectionNum - sectionNum + 1)
                time_table[key] = year_schedule

            year_schedule = time_table[key]
            year_schedule[indexId] = "".join(
                getCourseClass(cc, solution._configuration.getRoomById(roomId).Name, solution.criteria, ci)
            )
    ...
    def getHtmlCell(content, rowspan):
        ...
        if content is None:
            return "<td></td>"  # Возвращаем пустую ячейку вместо пустой строки
    ...
    def getTableHeader(year, spec, sectionNum):
        ...
        for weekDay in HtmlOutput.WEEK_DAYS:
            sb.append(
                "<th style='border: .1em solid black; padding: .25em; width: 15%' scope='col' colspan='" + str(
                    sectionNum) + "'>"
            )
            sb.append(weekDay)
            sb.append("</th>\n")
